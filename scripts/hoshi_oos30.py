...
跑与 hoshi_time_corr_windows.csv 完全相同的 30 个窗口（种子 20260907），
基线（原版 / h5s9）已有结果，本脚本只补跑候选方案。

用法: python hoshi_oos30.py <scheme>
  t5  分档仓位(9:1/8.5:.7/8:.45) + stop5 + 关R3 + 广度门控30
  s4  折中门槛8.5 + stop5 + 开R3 + 广度门控20
  t1  分档仓位 + stop5 + 开R3 + 广度门控20
"""
import csv
...
SCHEMES = {
    ...
    's4': dict(MIN_SCORE=8.5, LOSS_HARD_START_DAY=5, SCORE_TIER_SIZING=False,
               USE_R3_GATE=True, BREADTH_THRESH=20.0),
    't1': dict(MIN_SCORE=8.0, LOSS_HARD_START_DAY=5, SCORE_TIER_SIZING=True,
...
out_path = os.path.join(HERE, 'hoshi_oos30_%s.csv' % sk)
fout = open(out_path, 'w', newline='', encoding='utf-8')
wcsv = csv.writer(fout)
wcsv.writerow(['start', 'end', 'len_months', 'base_pct', 'h5s9_pct', '%s_pct' % sk, 'trades'])

print('方案 %s 开始，共 %d 个窗口' % (sk, len(WINDOWS)), flush=True)
for i, (s, e, m, b, h) in enumerate(WINDOWS, 1):
    r = hbe.run_backtest(code_bars, names, all_dates, start=s, end=e)
    ret = (r['final_capital'] - 500000.0) / 500000.0 * 100.0
    wcsv.writerow([s, e, m, '%.4f' % b, '%.4f' % h, '%.4f' % ret, len(r['closed'])])
    fout.flush()
    print('[%2d/%d] %s~%s (%2d月) 本方案 %+8.2f%% | 原版 %+8.2f%% | h5s9 %+8.2f%% | 笔数 %d'
          % (i, len(WINDOWS), s[:7], e[:7], m, ret, b, h, len(r['closed'])), flush=True)
