...
SCORE_TIER_SIZING = False   # 按评分分档决定单笔仓位系数（替代"要么8要么9"的二值门槛）
SCORE_TIERS = [(9.0, 1.00), (8.5, 0.70), (8.0, 0.45)]  # (评分下限, 仓位系数)，从高到低匹配
SCORE_TIER_ADAPTIVE = False  # 广度低时把各档系数整体收缩（防守）
SCORE_TIER_SHRINK = 0.5      # 广度低时的收缩倍数
...
def run_backtest(code_bars, names, all_dates, start=None, end=None):
    ...
    positions = {}
    closed = []
    buys = []  # 每笔买入事件（仅供复盘/实盘核对，不影响回测逻辑）
    equity_curve = []  # 每日 (date, 现金, 持仓市值, 权益)
    ...
    for today in dates:
        ...
        if signals and free_slots > 0:
            ...
            for score, code in signals:
                ...
                w = 1.0
                if SCORE_TIER_SIZING:
                    w = 0.0
                    for lo, f in SCORE_TIERS:
                        if score >= lo:
                            w = f
                            break
                    if w <= 0:
                        continue
                    if SCORE_TIER_ADAPTIVE and MARKET_FEAT:
                        mf = MARKET_FEAT.get(today)
                        if mf is not None and mf.get('breadth20', 100.0) * 100.0 < BREADTH_HI:
                            w *= SCORE_TIER_SHRINK
                budget = equity * MAX_POSITION_R * w
                ...
                buys.append({'date': today, 'code': code, 'name': names.get(code, ''),
                             'score': score, 'price': buy_price, 'shares': shares,
                             'cost': cost, 'mode': 'S4' if use_s4 else 'S3'})
                if COUNT_ENTRY_DAY:
        ...
        # 每日权益快照（仅供复盘/核对，不影响回测逻辑）
        mv_d = sum(p['shares'] * (bar_on(p['code'], today).close if bar_on(p['code'], today) else p['entry'])
                   for p in positions.values())
        equity_curve.append((today, capital, mv_d, capital + mv_d))
    ...
    return dict(closed=closed, final_capital=capital, stops=stops,
                skipped=skipped, n_s4=n_s4, buys=buys, equity_curve=equity_curve)
