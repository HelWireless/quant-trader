...
def _step_exit(pos, high, low, open_, close, allow_sell):
    ...
    if not armed:  # [对照实验] 全程硬止损：原版为 hd >= 15，前14天裸奔
        if not pos['loss_armed'] and low <= entry * (1 + LOSS_ARM_PCT / 100.0):
            pos['loss_armed'] = True
            pos['trough'] = low
        if pos['loss_armed']:
            pos['trough'] = min(pos['trough'], low)
            # [v3修复] 硬止损优先于反弹卖出：原实现先判反弹，导致深跌后
            # rebound=trough*1.022 仍是巨亏价（实测 -51%），硬止损 -8.2% 永远轮不到执行。
...
def main():
    global TOTAL_CAPITAL, MAX_SLOTS, EXIT_MODE, USE_BREADTH_GATE, USE_R3_GATE, ENFORCE_T1
