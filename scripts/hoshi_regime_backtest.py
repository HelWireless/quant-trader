...
LOSS_START_DAY = 15         # 从第几天开始监控止损(原版=15即第15天才止损,前14天裸奔;设1=全程止损兜底)
DEADLINE_DAY = 25           # 第25天未武装止盈则开盘价卖出

# ---- regime 条件式止损（实验新增，默认关闭=完全等同原版）----
USE_REGIME_LOSS = False     # True = 买入日按市场regime冻结该笔止损起始日; False = 恒用 LOSS_START_DAY(原版)
REGIME_INDEX_FILE = 'scripts/hoshi_regime/sh000001.csv'  # 市场regime判定用指数CSV(date,open,high,low,close)
REGIME_MA_FAST = 20         # 指数短均线
REGIME_MA_SLOW = 60         # 指数长均线
REGIME_LOSS_BULL = 15       # 多头/反弹市 用第15天止损(让盈利票跑,同原版)
REGIME_LOSS_WEAK = 7        # 弱势/震荡市 提前到第7天止损(尽早斩下跌票)
...
def _step_exit(pos, high, low, open_, close, allow_sell, regime_loss_day=None):
    """推进一个持仓一天的出场判定。pos 就地更新。返回 (卖价, 原因) 或 None。

    regime_loss_day: 当日有效止损起始日。None → 用持仓买入时冻结的 loss_start_day（恒 LOSS_START_DAY，原版）。
    一旦持仓达到当日 regime 对应起始日(hd>=regime_loss_day)，loss_eligible 永久置位(后续即使 regime 转多头也不再撤回止损监控)。
    """
    pos['hold_day'] += 1
    hd = pos['hold_day']
    entry = pos['entry']
    ...
    # 止损起始日：当日 regime 驱动(若启用)；一旦到达即永久置位
    eff_day = regime_loss_day if regime_loss_day is not None else pos.get('loss_start_day', LOSS_START_DAY)
    if not pos['loss_eligible'] and hd >= eff_day:
        pos['loss_eligible'] = True
    if pos['loss_eligible'] and not armed:
...
# ---- regime 判定辅助：从指数CSV预计算 每日 → 是否多头/反弹市 ----
def _load_index_regime(index_file, ma_fast=REGIME_MA_FAST, ma_slow=REGIME_MA_SLOW):
    """读指数CSV(date,open,high,low,close)，返回 {date: 1(多头/反弹) or 0(弱势/震荡)}。
    regime 定义：close > ma_fast 且 ma_fast > ma_slow → 多头/反弹(1)；否则弱势(0)。
    """
    ...
    for i, d in enumerate(dates):
        if i < ma_slow - 1:
            regime[d] = 0
            continue
        seg = dates[i - ma_slow + 1: i + 1]
        ma_s = sum(closes[x] for x in seg) / ma_slow
        ma_f = sum(closes[x] for x in seg[-ma_fast:]) / ma_fast
        regime[d] = 1 if (closes[d] > ma_f and ma_f > ma_s) else 0
    return regime
...
def run_backtest(code_bars, names, all_dates, start=None, end=None):
    ...
    # regime 条件式止损：预计算每个交易日的市场 regime (多头/反弹=1 用晚止损, 弱势=0 用早止损)
    regime_map = {}
    if USE_REGIME_LOSS:
        regime_map = _load_index_regime(REGIME_INDEX_FILE)
        _log('[regime] 已加载指数regime %d 个交易日 (多头=第%d天止损 / 弱势=第%d天止损)'
             % (len(regime_map), REGIME_LOSS_BULL, REGIME_LOSS_WEAK))

    def loss_day_on(today):
        """某交易日买入时该冻结的止损起始日。regime关闭→恒 LOSS_START_DAY。"""
        if not USE_REGIME_LOSS:
            return LOSS_START_DAY
        if today in regime_map:
            return REGIME_LOSS_BULL if regime_map[today] else REGIME_LOSS_WEAK
        # 指数缺该日数据 → 保守用较弱档(早止损)兜底
        return REGIME_LOSS_WEAK
    ...
    for today in dates:
        ...
        # 当日 regime 有效止损起始日(regime关=恒 LOSS_START_DAY)
        today_loss_day = loss_day_on(today)
        ...
        for code in list(positions.keys()):
            ...
            res = _step_exit(pos, b.high, b.low, b.open, b.close, allow_sell, regime_loss_day=today_loss_day)
            if res is None:
                continue
        ...
        if signals and free_slots > 0:
            ...
            for score, code in signals:
                ...
                positions[code] = {
                    ...
                    'score': score,
                    'loss_start_day': loss_day_on(today),   # regime 冻结的基准止损起始日(regime关=15)
                    'loss_eligible': False,                  # 止损监控是否已开启(一旦到日永久置位)
                }
                if COUNT_ENTRY_DAY:
...
def main():
    global TOTAL_CAPITAL, MAX_SLOTS, MAX_BUY_PER_DAY, EXIT_MODE, USE_BREADTH_GATE, USE_R3_GATE, ENFORCE_T1, LOSS_START_DAY, USE_REGIME_LOSS, REGIME_LOSS_WEAK, REGIME_LOSS_BULL
    ap = argparse.ArgumentParser(description='Hoshi 策略独立 CSV 回测器 (regime 条件式止损实验版)')
    ...
    ap.add_argument('--loss-start-day', type=int, default=LOSS_START_DAY,
                    help='从第几天起监控止损 (原版15; 设1=全程硬止损兜底)')
    ...
    ap.add_argument('--regime', action='store_true', help='开启 regime 条件式止损(买入日定止损起始日)')
    ap.add_argument('--regime-weak-day', type=int, default=REGIME_LOSS_WEAK,
                    help='弱势/震荡市的止损起始日(默认7)')
    ap.add_argument('--regime-bull-day', type=int, default=REGIME_LOSS_BULL,
                    help='多头/反弹市的止损起始日(默认15,同原版)')
    args = ap.parse_args()
    ...
    USE_REGIME_LOSS = args.regime
    REGIME_LOSS_WEAK = args.regime_weak_day
    REGIME_LOSS_BULL = args.regime_bull_day
    if args.no_t1:
        ENFORCE_T1 = False
